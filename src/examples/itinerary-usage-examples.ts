...
export class ItineraryUsageExamples {
  ...
  async basicItineraryExample(userId: string): Promise<ItineraryResponseDto> {
    ...
    const result = await this.itineraryAIService.generateItinerary(userId, generateDto);
  ...
  async budgetTripExample(userId: string): Promise<ItineraryResponseDto> {
    const generateDto: GenerateItineraryDto = {
      ...
      budget: '$500-800',
      specialRequests: 'Focus on free or low-cost activities, street food, and public transportation',
    };
    ...
    console.log(`- Activities per day: ${result.days.map(d => d.activities.length).join(', ')}`);
  ...
  async luxuryTripExample(userId: string): Promise<ItineraryResponseDto> {
    const generateDto: GenerateItineraryDto = {
      ...
      endDate: '2024-05-17',
      numberOfTravelers: 2,
      preferences: ['fine dining', 'luxury shopping', 'art museums', 'wine tasting'],
      tripType: TripType.LUXURY,
      budget: '$5000-8000',
      specialRequests: 'Include Michelin-starred restaurants, luxury hotels, private tours, and premium experiences',
    };
    ...
    result.days.forEach(day => {
      const highPriorityActivities = day.activities.filter(a => a.priority <= 2);
      console.log(`  Day ${day.dayNumber}: ${highPriorityActivities.length} must-do activities`);
    });
  ...
  async familyTripExample(userId: string): Promise<ItineraryResponseDto> {
    const generateDto: GenerateItineraryDto = {
      ...
      endDate: '2024-06-22',
      numberOfTravelers: 4,
      preferences: ['theme parks', 'family activities', 'kid-friendly restaurants'],
      tripType: TripType.MID_RANGE,
      budget: '$3000-4000',
      specialRequests: 'Family with 2 children (ages 8 and 12). Include theme park tickets, family-friendly accommodations, and activities suitable for kids.',
    };
    ...
    result.days.forEach(day => {
      const familyActivities = day.activities.filter(a => 
        a.tags.some(tag => ['family', 'kids', 'children'].includes(tag.toLowerCase()))
      );
      console.log(`  Day ${day.dayNumber}: ${familyActivities.length} family activities`);
    });
   ...
   */
  async getUserItinerariesExample(userId: string): Promise<ItineraryResponseDto[]> {
    ...
    itineraries.forEach((itinerary, index) => {
      console.log(`${index + 1}. ${itinerary.destination} (${itinerary.startDate} to ${itinerary.endDate})`);
      console.log(`   - ${itinerary.days.length} days, $${itinerary.totalEstimatedCost} estimated cost`);
      console.log(`   - Created: ${new Date(itinerary.createdAt).toLocaleDateString()}`);
    });
   ...
   */
  async analyzeItineraryStructure(itinerary: ItineraryResponseDto): void {
    console.log('\n=== Itinerary Analysis ===');
    ...
    console.log(`Temperature Range: ${itinerary.temperatureMin}°C - ${itinerary.temperatureMax}°C`);
    ...
    itinerary.days.forEach(day => {
      ...
      const categoryCounts = day.activities.reduce((acc, activity) => {
        acc[activity.category] = (acc[activity.category] || 0) + 1;
        return acc;
      }, {} as Record<string, number>);
      
      console.log('  Categories:', Object.entries(categoryCounts)
        .map(([cat, count]) => `${cat}(${count})`)
        .join(', '));
      
      const dailyCost = day.activities.reduce((sum, activity) => sum + (activity.estimatedCost || 0), 0);
    ...
    console.log(`Cost per Traveler: $${(totalCost / itinerary.numberOfTravelers).toFixed(2)}`);
    
    console.log('\n--- Activity Categories ---');
    const allActivities = itinerary.days.flatMap(day => day.activities);
    const categoryTotals = allActivities.reduce((acc, activity) => {
      acc[activity.category] = (acc[activity.category] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
